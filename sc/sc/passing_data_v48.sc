...
//0
main (%d385 NOP(0,0) %d0)
data_center (%d11 NOP(0,0) %d0)
data1 (%d4 NOP(0,0) %d10)
data1 (%d21 NOP(0,0) %d100)
...
#chain data_mark
{
($L output $R) + ($L rec ?A) + (A tagf $R) + (?B read_main ?C) + ($L tst_output A) + ($L res $R) + ($L tst_output $R) + ($L incrementor $R) + ($L tst_output $R) + ($L esc $R)
}

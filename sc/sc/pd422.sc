...
esc ([num zero dontcare] NOP(0,0) [num zero dontcare])
...
fit_output ([dontcare zero dontcare] PRINT(0,0) [dontcare zero dontcare])
...
#chain data_mark
{
($L output $R) + ($L rec ?A) + ($L fit_output A) + ($L res $R)
}

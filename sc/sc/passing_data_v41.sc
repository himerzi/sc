...
// Original source: /Users/md/Documents/UCL 3/final year project/code/sc/sc/passing_data_v4.sc
...
//0
main (%d0 NOP(0,0) %d0)
data_center (%d0 NOP(0,0) %d0)
data1 (%d3 NOP(0,0) %d1)
data1 (%d20 NOP(0,0) %d1)
...
tagf ([dontcare zero dontcare] TAG_FITNESS(0,0) [dontcare zero dontcare])
...
#chain data_mark
{
($L output $R) + ($L rec ?A) + (A tagf $R) + ($L tst_output A) + ($L res $R) + ($L tst_output $R) + ($L incrementor $R) + ($L tst_output $R) + ($L esc $R)
}
